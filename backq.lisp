...
(defun backquote-macro (stream ignore)
  ...
  (let ((*backquote-count* (1+ *backquote-count*)))
    (multiple-value-bind (flag thing)
      (backquotify stream (cl-read stream t nil t))
...
(defun comma-macro (stream ignore)
  ...
  (let ((c (read-char stream))
    ...
    (cond ((char= c #\@)
           (cons *bq-at-flag* (cl-read stream t nil t)))
      ((char= c #\.)
       (cons *bq-dot-flag* (cl-read stream t nil t)))
      (t (unread-char c stream)
         (cons *bq-comma-flag* (cl-read stream t nil t))))))
...
(defun !backq-cold-init ()
  (cl-set-macro-character #\` #'backquote-macro)
  (cl-set-macro-character #\, #'comma-macro))
(!backq-cold-init)
