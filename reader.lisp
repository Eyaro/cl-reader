...
(defvar *cl-readtable* (make-cl-readtable))
(declaim (type cl-readtable *cl-readtable*))

#| 
(setf (fdocumentation '*cl-readtable* 'variable)
...
(defvar *cl-standard-readtable* nil)
...
(defun set-cat-entry (char newvalue &optional (rt *cl-readtable*))
  (declare (type cl-readtable rt))
  (if (typep char 'base-char)
...
;;; ANSI #'CL-GET-MACRO-CHARACTER and #'CL-SET-MACRO-CHARACTER, this can
;;; be either a function or NIL.
(defun get-raw-cmt-entry (char readtable)
  (declare (type cl-readtable readtable))
  (if (typep char 'base-char)
      (svref (character-macro-array readtable) (char-code char))
      ;;TODO: Undefined macro char and the second case???
...
;;; table. As per ANSI #'CL-GET-MACRO-CHARACTER and #'CL-SET-MACRO-CHARACTER,
...
(defun set-cmt-entry (char new-value-designator &optional (rt *cl-readtable*))
  (let ((new (when new-value-designator
               (assert (functionp new-value-designator))
               new-value-designator)))
    (if (typep char 'base-char)
...
(defun whitespace[1]p (char)
  (test-attribute char +char-attr-whitespace+ *cl-standard-readtable*))
(defun whitespace[2]p (char &optional (rt *cl-readtable*))
  (test-attribute char +char-attr-whitespace+ rt))

(defun constituentp (char &optional (rt *cl-readtable*))
  (test-attribute char +char-attr-constituent+ rt))

(defun terminating-macrop (char &optional (rt *cl-readtable*))
  (test-attribute char +char-attr-terminating-macro+ rt))

(defun single-escape-p (char &optional (rt *cl-readtable*))
  (test-attribute char +char-attr-single-escape+ rt))

(defun multiple-escape-p (char &optional (rt *cl-readtable*))
  (test-attribute char +char-attr-multiple-escape+ rt))

(defun token-delimiterp (char &optional (rt *cl-readtable*))
...
(defun !cold-init-constituent-trait-table ()
  ...
  (dolist (c (list 
              (char-code #\Backspace) (char-code #\Tab) (char-code #\Page)
...
(defun assert-not-standard-readtable (readtable operation)
  (when (eq readtable *cl-standard-readtable*)
    (cerror "Frob it anyway!" 'standard-readtable-modified-error
            :operation operation)))

(defun cl-readtable-case (readtable)
  (%readtable-case readtable))

(defun (setf cl-readtable-case) (case readtable)
  (assert-not-standard-readtable readtable '(setf cl-readtable-case))
  (setf (%readtable-case readtable) case))

(defun shallow-replace/eql-hash-table (to from)
  (maphash (lambda (k v) (setf (gethash k to) v)) from))

(defun cl-copy-readtable (&optional (from-readtable *cl-readtable*) to-readtable)
  (assert-not-standard-readtable to-readtable 'cl-copy-readtable)
  (let ((really-from-readtable (or from-readtable *cl-standard-readtable*))
        (really-to-readtable (or to-readtable (make-cl-readtable))))
    ...
    (setf (cl-readtable-case really-to-readtable)
          (cl-readtable-case really-from-readtable))
    really-to-readtable))


(defun cl-set-syntax-from-char (to-char from-char
                                        &optional (to-readtable *cl-readtable*) (from-readtable nil))
...
standard Lisp readtable when NIL."
  (assert-not-standard-readtable to-readtable 'cl-set-syntax-from-char)
  (let ((really-from-readtable (or from-readtable *cl-standard-readtable*)))
...
(defun cl-set-macro-character (char function &optional
                                    (non-terminatingp nil)
                                    (rt-designator *cl-readtable*))
...
character non-terminating, i.e. embeddable in a symbol name."
  (let ((designated-readtable (or rt-designator *cl-standard-readtable*)))
    (assert-not-standard-readtable designated-readtable 'cl-set-macro-character)
...
;;TODO:Change ANSI standard
(defun cl-get-macro-character (char &optional (rt-designator *cl-readtable*))
...
(defun cl-make-dispatch-macro-character (char &optional
...
be non-terminating."
  ...
  (cl-set-macro-character char #'read-dispatch-char non-terminating-p rt)
...
(defun cl-set-dispatch-macro-character (disp-char sub-char function
                                                  &optional (rt-designator *cl-readtable*)) 
...
followed by SUB-CHAR."
  ...
  (let* ((sub-char (char-upcase sub-char))
         (readtable (or rt-designator *cl-standard-readtable*)))
    (assert-not-standard-readtable readtable 'cl-set-dispatch-macro-character)
...
(defun cl-get-dispatch-macro-character (disp-char sub-char
                                                  &optional (rt-designator *cl-readtable*))
...
(defun flush-whitespace (stream)
  ...
  (let ((stream (in-synonym-of stream)))
    ;; CLOS stream
    (do ((attribute-array (character-attribute-array *cl-readtable*))
         (attribute-hash-table
          (character-attribute-hash-table *cl-readtable*))
...
;; Install the (easy) standard macro-chars into *cl-readtable*.
(defun !cold-init-standard-readtable ()
  ...
  (flet ((whitespaceify (char)
    ...
    (whitespaceify #\Space)
    ;;TODO: Always page/return???
    (whitespaceify #\Page)
    (whitespaceify #\Return))
  ...
  (cl-set-macro-character #\" #'read-string)
  (cl-set-macro-character #\' #'read-quote)
  (cl-set-macro-character #\( #'read-list)
  (cl-set-macro-character #\) #'read-right-paren)
  (cl-set-macro-character #\; #'read-comment)
...
(defvar *inch-ptr*) ; *INCH-PTR* always points to next char to read.
(defvar *ouch-ptr*) ; *OUCH-PTR* always points to next char to write. 

(declaim (type integer *inch-ptr* *ouch-ptr*))
...
(defun ouch-read-buffer (char)
  ...
  (let ((op *ouch-ptr*))
    ;;TODO: sb!c::insert-array-bounds-checks
  ...
  (if recursive-p
      ...
      (loop
       (let ((char (read-char stream eof-error-p *eof-object*)))
         (cond 
           ((eofp char) (return eof-value))
           ((whitespace[2]p char))
           (t
            (let* ((macrofun (get-coerced-cmt-entry char *cl-readtable*))
...
(defun cl-read-preserving-whitespace (&optional (stream *standard-input*)
                                                (eof-error-p t)
                                                (eof-value nil)
                                                (recursive-p nil))
...
that followed the object."
  (check-for-recursive-read stream recursive-p 'cl-read-preserving-whitespace)
...
(defun read-maybe-nothing (stream char)
  (let ((retval (multiple-value-list
                 (funcall (get-coerced-cmt-entry char *cl-readtable*)
                          stream
                          char))))
...
(defun cl-read (&optional (stream *standard-input*)
  ...
  (check-for-recursive-read stream recursive-p 'cl-read)
...
(defun cl-read-delimited-list (endchar &optional
                                       (input-stream *standard-input*)
                                       recursive-p)
  "Read Lisp values from INPUT-STREAM until the next character after a
value's representation is ENDCHAR, and return the objects as a list."
  (check-for-recursive-read input-stream recursive-p 'cl-read-delimited-list)
...
(defun read-comment (stream ignore)
  ...
  (handler-bind nil
   ...
   (style-warn
   ;;TODO: sb!kernel::character-decoding-error-in-macro-char-comment
   'TODO ;; 'sb!kernel::character-decoding-error-in-macro-char-comment
      ...
      (t
       (macrolet ((skip-esc (&body body)
                            `(do ((i (1- *ouch-ptr*) (1- i))
                                 ...
                                 ((minusp i))
                               (declare (fixnum i))
                               ;;TODO (optimize (sb!c::insert-array-bounds-checks 0))
                               ;(declare (fixnum i)
...
(defun read-token (stream firstchar)
  ...
  (let ((attribute-array (character-attribute-array *cl-readtable*))
        (attribute-hash-table (character-attribute-hash-table *cl-readtable*))
    ...
    (prog ((char firstchar))
          ...
          (case (char-class char attribute-array attribute-hash-table)
            ...
            (#.+char-attr-constituent-slash+ (assert (not possibly-rational))
                                             (go SYMBOL))
            (#.+char-attr-delimiter+ (unread-char char stream)
                                     (go RETURN-SYMBOL))
            (#.+char-attr-single-escape+ (go SINGLE-ESCAPE))
...
(defun read-extended-token (stream &optional (*cl-readtable* *cl-readtable*))
...
(defun read-extended-token-escaped (stream &optional (*cl-readtable* *cl-readtable*))
...
then multiplying by a power of the base and adding."
  (let* ((base *read-base*)
         ...
         (negativep nil)
         (number 0))
    (declare (type integer digits-per base-power))
    ...
    (loop
     (let ((num 0))
       (declare (type integer num)) ;;TODO: index?
...
(defun read-dispatch-char (stream char)
  ...
  (let ((numargp nil)
    ...
    (let ((dpair (find char (dispatch-tables *cl-readtable*)
                       :test #'char= :key #'car)))
      (if dpair
  ...
  (with-array-data ((string string :offset-var offset)
    ...
    (let ((stream (make-string-input-stream string start end)))
      (values (if preserve-whitespace
              ...
              ;;TODO: (string-input-sream-current stream) ????
  ...
  (macrolet ((parse-error (format-control)
    ...
    (with-array-data ((string string :offset-var offset)
      ...
      (let ((index (do ((i start (1+ i)))
                       ((= i end)
                        (if junk-allowed
                            (return-from cl-parse-integer (values nil end))
...
(!reader-cold-init)
;;TODO: print-object cl-readtable
#|  
